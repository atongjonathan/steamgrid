...
const BannerItem: React.FC<{ movie: BannerMovie }> = ({ movie }) => {
    ...
    useEffect(() => {
        ...
        if (isActive) {
            ...
            timers.push(setTimeout(() => setPhases(prev => ({ ...prev, info: true })), 750));
            timers.push(setTimeout(() => setPhases(prev => ({ ...prev, plot: true })), 1000));
            timers.push(setTimeout(() => setPhases(prev => ({ ...prev, buttons: true })), 1250));
        } else {
        ...
        <>
            ...
            {/* Metadata Phases */}
                <div className="lg:w-7/12 absolute linear-bg xl:pl-32 sm:pl-32 pl-8 top-0 bottom-0 right-0 left-0 flex flex-col justify-center lg:gap-8 md:gap-5 gap-4 z-20 md:h-96 min-h-80">
                    ...
                    {/* Buttons */}
                    <div className={`flex gap-5 items-center transition-opacity duration-700 text-white ${phases.buttons ? 'opacity-100' : 'opacity-0'}`}>
                        <Button asChild>
                            <Link
                                to="/history"
