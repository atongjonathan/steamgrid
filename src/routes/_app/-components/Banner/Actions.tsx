...
import { updateUser, type BannerMovie, type ResponseUser, type UserUpdatePayload } from '@/api';
...
const PowerButton2: React.FC<{ actions: Action[] }> = ({ actions }) => {
    ...
    return (
        <PopoverTrigger asChild>
            <Button
                variant="ghost"
                className="px-2 py-1 text-white bg-white/10 hover:bg-white/20 rounded-lg text-sm flex items-center gap-2"
            >
...
const Actions: React.FC<{ movie: BannerMovie }> = ({ movie }) => {
    const { user } = useAuth();
    const { pathname } = useLocation();
    const id = movie?.id;

    const actions: Action[] = [
        { Icon: MdOutlineBookmark, label: 'Saved', name: 'plan', enabled: !!user?.plan?.some((m) => m.id === id) },
        { Icon: MdOutlineStopCircle, label: 'On Hold', name: 'hold', enabled: !!user?.hold?.some((m) => m.id === id) },
        { Icon: IoMdClose, label: 'Dropped', name: 'dropped', enabled: !!user?.dropped?.some((m) => m.id === id) },
        { Icon: RiCheckDoubleFill, label: 'Finished', name: 'finished', enabled: !!user?.finished?.some((m) => m.id === id) }
    ];
...
const ActionItem: React.FC<ActionItemProps> = ({ Icon, label, name, id, enabled }) => {
    const { user, fetchUser } = useAuth();
    const queryClient = useQueryClient();

    const { mutate, isPending } = useMutation({
        mutationKey: ['updateMe'],
        mutationFn: async (data: Partial<Record<"plan_ids" | "hold_ids" | "dropped_ids" | "finished_ids", number[]>>) => {
            if (!user) throw new Error("No user");

            const { plan = [], hold = [], finished = [], dropped = [], favourites = [], image, ...baseUser } = user;
    ...
    const handleClick = () => {
        ...
        const update: UserUpdatePayload = {
            ...
            dropped_ids: user.dropped?.filter((m) => m.id !== id).map((m) => m.id) || [],
            finished_ids: user.finished?.filter((m) => m.id !== id).map((m) => m.id) || [],
        };
